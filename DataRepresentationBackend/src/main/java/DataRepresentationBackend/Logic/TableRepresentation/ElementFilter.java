...
public class ElementFilter {
    ...
    public ArrayList<Wall> getWalls() throws DataNotValidException {/**Filters from the set of TableElements those which are Walls*/
        ...
        for (int i = 0; i < this.elements.size(); i++) {
            ...
            if ((element.isExit == 1) && (element.isExterior == 0))
                DataNotValidExceptionLogger.getInstance().addExceptionMessage(element + " is an Exit but it is not Exterior !");
            if ((element.isExit == 1) && (!element.elementType.equalsIgnoreCase("door")))
                DataNotValidExceptionLogger.getInstance().addExceptionMessage(element + " is an Exit but it is not a Door");
